...
((Drupal, once) => {
  ...
  'use strict';
  Drupal.worldcampus = Drupal.worldcampus || {};
  Drupal.worldcampus.scrollOffsetCallbacks = Drupal.worldcampus.scrollOffsetCallbacks || [];

  /**
   * Calculates the total height to compensate for when scrolling.
   */
  Drupal.worldcampus.revalidateScrollOffset = function(additional) {
    // Add up the sum of all the callbacks.
    const callbacks = Drupal.worldcampus.scrollOffsetCallbacks;
    ...
    attach: context => {
      ...
      jump_links.forEach(jump_link => {
        jump_link.addEventListener('click', () => {
          ...
          Drupal.worldcampus.revalidateScrollOffset(additional);
        });
      });

      // Only try to calculate things after all JS has had a chance to run.
      setTimeout(function() {
        Drupal.worldcampus.revalidateScrollOffset();
      }, 1);
    }
  };
})(Drupal, once);
